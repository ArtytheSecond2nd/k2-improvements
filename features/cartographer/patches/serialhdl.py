...
class SerialReader:
    ...
    def check_connect(self, serialport, baud, rts=True):
        """Test if serial port can be opened. Returns True if available, False otherwise."""
    ...
    def _check_noncritical_disconnected(self):
        """Guard that raises error if non-critical MCU is disconnected.
        
        Skip check if MCU is in reconnection mode (attempting to reconnect).
        """
        if self.mcu is None:
            return
        # Skip check if MCU is attempting to reconnect
        if getattr(self.mcu, '_reconnecting', False):
            return
